...
function App() {
  ...
  // [स्टेप 4 नया स्टेट] यूजर की पर्सनल पासबुक/स्टेटमेंट हिस्ट्री के लिए
  const [userStatement, setUserStatement] = useState([
    { id: "st1", userId: "u1", type: "CREDIT", title: "Welcome Bonus", amount: "10 Coins", date: "Initial" }
  ]);
  ...
  useEffect(() => {
    ...
    const storedStatements = localStorage.getItem('phone_statements');
    ...
    if (storedStatements) setUserStatement(JSON.parse(storedStatements));
  }, []);

  const saveToStorage = (b, c, u, s, st) => {
    ...
    localStorage.setItem('phone_statements', JSON.stringify(st));
  };
  ...
  const handleAuth = (e) => {
    ...
    if (isSignUp) {
      ...
      const newUser = { id: newId, username: regUsername, email: regEmail, coins: 10, unlockedChapters: ["ch1"] };
      const updatedUsers = [...users, newUser];
      
      const newSt = { id: 'st_' + Date.now(), userId: newId, type: "CREDIT", title: "Sign Up Reward", amount: "10 Coins", date: "Just now" };
      const updatedSt = [newSt, ...userStatement];
      
      setUsers(updatedUsers); 
      setUserStatement(updatedSt);
      setCurrentUser(newUser);
      saveToStorage(books, chapters, updatedUsers, salesLog, updatedSt); 
      setView('home');
    } else {
  ...
  const handleDeleteBook = (bookId, e) => {
    ...
    setBooks(updatedBooks); setChapters(updatedChapters);
    saveToStorage(updatedBooks, updatedChapters, users, salesLog, userStatement);
  ...
  const handleModifyCoins = (actionType) => {
    ...
    // यूजर स्टेटमेंट अपडेट करें
    const newSt = { id: 'st_' + Date.now(), userId: adminCoinTarget, type: actionType === 'ADD' ? "CREDIT" : "DEBIT", title: actionType === 'ADD' ? "Vault Load by Admin" : "Vault Deduct by Admin", amount: `${amount} Coins`, date: "Just now" };
    const updatedSt = [newSt, ...userStatement];
  ...
  const handleApproveNovel = (novelId) => {
    ...
    if (novelToApprove) {
      ...
      saveToStorage(updatedBooks, chapters, users, salesLog, userStatement);
  ...
  const handleStartPayment = (pack) => {
    ...
    setPaymentPopup(pack); setPaymentStatus('idle');
  };
  ...
  const handleProcessPayment = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const newSt = { id: 'st_' + Date.now(), userId: currentUser.id, type: "CREDIT", title: `Shop: ${paymentPopup.name}`, amount: `+${paymentPopup.coins} Coins`, date: "Just now" };
        const updatedSt = [newSt, ...userStatement];

        setUserStatement(updatedSt);
        setSalesLog(updatedSales);
        setUsers(updatedUsers);
        saveToStorage(books, chapters, updatedUsers, updatedSales, updatedSt);
  ...
  const handleUnlockChapter = (ch) => {
    ...
    if (currentUser.coins >= ch.price) {
      ...
      const newSt = { id: 'st_' + Date.now(), userId: currentUser.id, type: "DEBIT", title: `Unlocked Chapter ${ch.number}`, amount: `-${ch.price} Coins`, date: "Just now" };
      const updatedSt = [newSt, ...userStatement];
  ...
  const handleCreateBook = (e) => {
    ...
    const updated = [...books, nBook]; setBooks(updated); saveToStorage(updated, chapters, users, salesLog, userStatement);
  ...
  const handleCreateChapter = (e) => {
    ...
    setChapters(updated); saveToStorage(books, updated, users, salesLog, userStatement);
  ...
  const handleChapterComplete = (ch) => {
    ...
    const newSt = { id: 'st_' + Date.now(), userId: currentUser.id, type: "CREDIT", title: `Finished Chapter ${ch.number}`, amount: `+${ch.reward} Coins`, date: "Just now" };
    const updatedSt = [newSt, ...userStatement];
    ...
    <div className="min-h-screen bg-[#07070a] text-gray-100 font-sans pb-12 antialiased selection:bg-purple-500/30">
      ...
      <nav className="bg-[#0f0f16]/90 backdrop-blur-md px-4 py-3 flex justify-between items-center border-b border-purple-900/30 sticky top-0 z-40 shadow-xl shadow-black/20">
        ...
        <div className="flex items-center gap-2 text-xs">
          <button onClick={() => setView('coin-shop')} className="bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2.5 py-1.5 rounded-xl font-bold flex items-center gap-1 transition-all">
            <i className="fa-solid fa-coins text-amber-500"></i> Shop
          </button>
          {currentUser?.role === 'owner' && (
            <button onClick={() => setView('admin')} className="bg-purple-600 hover:bg-purple-700 text-white px-2.5 py-1.5 rounded-xl font-bold transition-all shadow-lg shadow-purple-600/10">Studio</button>
          )}
          {currentUser ? (
            <div onClick={() => setView('profile')} className="flex items-center gap-2 bg-[#161622] border border-gray-800/80 px-2.5 py-1.5 rounded-xl cursor-pointer hover:border-purple-500/30 transition-all">
              <i className="fa-solid fa-circle-user text-purple-400 text-sm"></i>
              <span className="text-amber-400 font-black">🪙 {currentUser.coins || 0}</span>
            </div>
          ) : (
            <button onClick={() => setView('login')} className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-3 py-1.5 rounded-xl font-bold transition-all">Login</button>
          )}
        </div>
      </nav>
      ...
      <main className="p-4 max-w-xl mx-auto">
                ...
                <div className="flex flex-col justify-between py-1 flex-1">
                  <div>
                    <div className="flex justify-between items-start">
                      ...
                      {currentUser?.role === 'owner' && (
                        <button onClick={(e) => handleDeleteBook(b.id, e)} className="text-rose-500 hover:text-rose-400 text-xs p-1.5 z-10 rounded-lg hover:bg-rose-950/30 transition-all">
        ...
        {view === 'book-details' && selectedBook && (
          <div className="space-y-5 animate-fade-in">
            ...
            <div className="bg-[#0f0f16] p-5 rounded-3xl border border-purple-900/10 text-center shadow-2xl relative">
                ...
                {chapters.filter(c => c.bookId === selectedBook.id).map(ch => {
                  ...
                  return (
                    <div key={ch.id} className="p-3 bg-[#13131c] rounded-xl border border-purple-900/5 flex justify-between items-center text-xs hover:border-purple-500/20 transition-all">
                      ...
                      {isUnlocked ? (
                        <button onClick={() => { setSelectedChapter(ch); setView('reader'); }} className="bg-purple-600 hover:bg-purple-700 px-4 py-1.5 rounded-lg font-bold shadow-md text-white transition-all">Read</button>
                      ) : (
                        <button onClick={() => handleUnlockChapter(ch)} className="bg-gradient-to-r from-amber-500 to-yellow-500 text-black px-3 py-1.5 rounded-lg font-black shadow flex items-center gap-1 transition-all">
                          Unlock ({ch.price} 🪙)
                        </button>
                      )}
                    </div>
        ...
        {view === 'reader' && selectedChapter && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-[#0f0f16] p-6 rounded-3xl border border-purple-900/20 shadow-2xl">
              ...
              <button onClick={() => handleChapterComplete(selectedChapter)} className="w-full bg-gradient-to-r from-amber-500 to-yellow-500 text-black font-black p-3 rounded-2xl text-xs shadow-xl uppercase tracking-widest transition-all">
              ...
              {COIN_PACKS.map(pack => (
                <div key={pack.id} className="bg-[#0f0f16] p-4 rounded-2xl border border-purple-900/10 flex justify-between items-center shadow-xl hover:border-purple-500/20 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-[#14141f] rounded-xl flex items-center justify-center text-xl shadow-inner"><i className={pack.icon}></i></div>
                    <div>
                      <h3 className="font-bold text-white text-sm">{pack.name}</h3>
                      <p className="text-xs text-amber-400 font-black mt-0.5">+{pack.coins} Coins Pack</p>
                    </div>
                  </div>
                  <button onClick={() => handleStartPayment(pack)} className="bg-purple-600 hover:bg-purple-700 text-white font-black px-5 py-2.5 rounded-xl text-xs transition-all">INR {pack.price}</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* [स्टेप 4 नया UI] यूजर प्रोफाइल, वॉलेट लेजर और लाइब्रेरी */}
        {view === 'profile' && currentUser && (
          <div className="space-y-5 anonymity-fade-in text-xs">
            <div className="flex justify-between items-center">
              <button onClick={() => setView('home')} className="text-xs text-gray-400 hover:text-white transition-colors">&larr; Back to Home</button>
              <button onClick={() => { setCurrentUser(null); setView('home'); }} className="bg-rose-950 text-rose-400 border border-rose-900/40 px-3 py-1.5 rounded-xl font-bold hover:bg-rose-900/50 transition-all">
                Logout Account
              </button>
            </div>

            {/* Profile Hero Card */}
            <div className="bg-gradient-to-br from-[#0f0f16] to-[#141424] p-5 rounded-3xl border border-purple-500/10 flex items-center gap-4 shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-5 text-4xl"><i className="fa-solid fa-id-card-clip"></i></div>
              <div className="w-14 h-14 bg-purple-600/20 border border-purple-500/30 rounded-2xl flex items-center justify-center text-2xl text-purple-400 shadow-xl shadow-black/30">
                <i className="fa-solid fa-user-astronaut"></i>
              </div>
              <div>
                <h2 className="text-base font-black text-white tracking-wide">{currentUser.username}</h2>
                <p className="text-[11px] text-gray-400 mt-0.5">{currentUser.email}</p>
                <div className="inline-block bg-amber-500/10 text-amber-400 px-2.5 py-0.5 rounded-lg border border-amber-500/20 font-black mt-2 text-[10px]">
                  Vault Balance: 🪙 {currentUser.coins || 0}
                </div>
              </div>
            </div>

            {/* User Library Shortcut (Unlocked Content) */}
            <div className="bg-[#0f0f16] p-4 rounded-3xl border border-purple-900/10 shadow-xl space-y-3">
              <h3 className="text-xs font-black text-purple-400 uppercase tracking-widest flex items-center gap-1.5">
                <i className="fa-solid fa-folder-open text-purple-500"></i> My Unlocked Library
              </h3>
              <div className="space-y-2">
                {books.map(b => {
                  const unlockedChaptersCount = chapters.filter(c => c.bookId === b.id && currentUser.unlockedChapters?.includes(c.id)).length;
                  if (unlockedChaptersCount === 0 && currentUser.role !== 'owner') return null;
                  return (
                    <div key={b.id} onClick={() => { setSelectedBook(b); setView('book-details'); }} className="p-3 bg-[#14141f] border border-gray-800 rounded-xl flex justify-between items-center cursor-pointer hover:border-purple-500/20 transition-all shadow-inner">
                      <span className="font-bold text-gray-200">{b.title}</span>
                      <span className="bg-purple-950/60 text-purple-400 font-bold px-2 py-0.5 rounded-lg text-[10px]">
                        {currentUser.role === 'owner' ? "All Access" : `${unlockedChaptersCount} Chapters Unlocked`}
                      </span>
                    </div>
                  );
                })}
            ...
            {/* Wallet Statement / Passbook History */}
            <div className="bg-[#0f0f16] p-5 rounded-3xl border border-purple-900/10 shadow-2xl space-y-3">
              <h3 className="text-xs font-black text-amber-400 uppercase tracking-widest flex items-center gap-1.5">
                <i className="fa-solid fa-receipt text-amber-500"></i> Wallet Passbook Statement
              </h3>
              <div className="max-h-48 overflow-y-auto space-y-2 pr-1">
                {userStatement.filter(st => st.userId === currentUser.id).length === 0 ? (
                  <p className="text-gray-500 italic text-[11px] text-center py-4">No recent statement records found.</p>
                ) : (
                  userStatement.filter(st => st.userId === currentUser.id).map(st => (
                    <div key={st.id} className="p-3 bg-[#14141f] rounded-xl border border-purple-900/5 flex justify-between items-center shadow-inner">
                      <div>
                        <p className="text-white font-bold">{st.title}</p>
                        <p className="text-gray-500 text-[10px] mt-0.5">{st.date}</p>
                      </div>
                      <span className={`font-black px-2.5 py-0.5 rounded-lg text-[11px] ${st.type === 'CREDIT' ? 'bg-emerald-950/30 text-emerald-400 border border-emerald-900/40' : 'bg-rose-950/30 text-rose-400 border border-rose-900/40'}`}>
                        {st.amount}
                      </span>
                    </div>
                  ))
        ...
        {/* Master Control Admin Dashboard View */}
        {view === 'admin' && currentUser?.role === 'owner' && (
          <div className="space-y-6 text-xs animate-fade-in">
            ...
            <div className="bg-[#11111a] p-5 rounded-3xl border border-amber-500/20 shadow-2xl space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <select 
                  ...
                  className="w-full bg-[#0a0a0f] border border-gray-800 p-2.5 rounded-xl text-white font-bold outline-none focus:border-amber-500/50"
                >
                  {users.map(u => (
              ...
              <div className="flex items-center gap-2">
                <input type="number" placeholder="Amount" value={coinModifyAmount || ''} onChange={e => setCoinModifyAmount(e.target.value)} className="w-24 bg-[#0a0a0f] border border-gray-800 p-2.5 rounded-xl text-white text-center font-black outline-none" />
                <button onClick={() => handleModifyCoins('ADD')} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl shadow-lg transition-all">+ Issue Coins</button>
                <button onClick={() => handleModifyCoins('DEDUCT')} className="flex-1 bg-rose-600 hover:bg-rose-700 text-white font-bold py-2.5 rounded-xl shadow-lg transition-all">- Remove Coins</button>
              </div>
            </div>
              ...
              <div className="max-h-40 overflow-y-auto space-y-2 pr-1">
                {salesLog.map(log => (
                  <div key={log.id} className="p-3 bg-[#14141f] rounded-xl border border-purple-900/5 flex justify-between items-center shadow-inner">
                    ...
                    <span className="text-emerald-400 font-black bg-emerald-950/20 border border-emerald-900/30 px-3 py-1 rounded-lg">{log.price}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* Creator Submission queue */}
            <div className="bg-[#0f0f16] p-5 rounded-3xl border border-purple-900/10 shadow-2xl space-y-3">
              ...
              {pendingNovels.map(pn => (
                <div key={pn.id} className="p-3 bg-[#14141f] border border-gray-800 rounded-xl flex justify-between items-center shadow-inner">
                  ...
                  <button onClick={() => handleApproveNovel(pn.id)} className="bg-purple-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-black shadow transition-all">Approve</button>
                </div>
              ))}
            </div>
            ...
            <div className="bg-[#0f0f16] p-4 rounded-3xl border border-purple-900/10 space-y-2 shadow-2xl">
              ...
              <input type="number" placeholder="Chapter Number" value={newChNumber} onChange={e=>setNewChNumber(e.target.value)} className="w-full bg-[#14141f] p-2.5 rounded-xl text-white border border-gray-800" />
              <input type="text" placeholder="Chapter Title" value={newChTitle} onChange={e=>setNewChTitle(e.target.value)} className="w-full bg-[#14141f] p-2.5 rounded-xl text-white border border-gray-800" />
              ...
              {newChIsLocked && <input type="number" placeholder="Price (Coins)" value={newChPrice} onChange={e=>setNewChPrice(e.target.value)} className="w-full bg-[#14141f] p-2.5 rounded-xl text-white border border-gray-800" />}
              <input type="number" placeholder="Read Completion Reward Coins" value={newChReward} onChange={e=>setNewChReward(e.target.value)} className="w-full bg-[#14141f] p-2.5 rounded-xl text-white border border-gray-800" />
        ...
        {view === 'login' && (
          <div className="bg-[#0f0f16] p-6 rounded-3xl border border-purple-900/10 shadow-2xl space-y-4 text-xs animate-fade-in">
            ...
            <button onClick={() => setIsSignUp(!isSignUp)} className="text-purple-400 block mx-auto mt-3 font-semibold text-[11px]">{isSignUp ? "Already have an account? Sign In" : "New creator or reader? Sign Up Now"}</button>
          </div>
        )}
      </main>

      {/* Razorpay Simulated Popup */}
      {paymentPopup && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-[#0f0f16] border border-purple-500/20 w-full max-w-xs rounded-3xl p-5 shadow-2xl space-y-4 text-center relative">
            <div className="flex justify-between items-center border-b border-purple-900/10 pb-2.5">
              ...
              <button onClick={() => setPaymentPopup(null)} className="text-gray-500 hover:text-white text-sm">&times;</button>
            </div>
            {paymentStatus === 'idle' && (
              <div className="space-y-4 py-1">
                ...
                <div className="p-3 bg-[#14141f] rounded-xl border border-purple-900/5 flex justify-between items-center text-xs">
                  ...
                  <span className="text-amber-400 font-black">🪙 {paymentPopup.coins} Pack</span>
                </div>
                <button onClick={handleProcessPayment} className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-black py-3 rounded-xl text-xs shadow-xl uppercase tracking-wider">Proceed To Pay (Simulated)</button>
              </div>
            )}
            ...
            {paymentStatus === 'success' && (
              <div className="py-6 space-y-2.5">
                <div className="w-12 h-12 bg-emerald-950/40 text-emerald-400 rounded-full flex items-center justify-center text-xl mx-auto border border-emerald-500/30 shadow-xl"><i className="fa-solid fa-circle-check"></i></div>
